...
- 输入框内审批模式左侧以“上下文 24%”显示最近一次模型调用的上下文占用；点击可查看完整统计、System Prompt、Messages、Tools 和 Token 估算明细。
- 每个会话的最新上下文快照保存在会话目录，重启后仍可查看；尚未产生快照时隐藏入口，删除会话时一并删除快照。
- 当工具需要审批时，在聊天消息的工具块中点击“批准”或“拒绝”；批准后原任务会自动继续执行。
- 工具调用区域会显示执行中、成功、失败和待审批状态；长任务会持续显示已执行时长。
- 计划面板先展示“当前目标”，再展示步骤和进度。目标随计划保存，调整步骤或恢复计划不会改变目标；旧计划没有目标时仍正常展示步骤。历史计划以默认收起的轻量任务记录展示目标、状态和完成数，点击可展开步骤详情。
- 计划进度只在本轮执行或等待审批时显示在输入框上方；等待用户、完成或失败后保留在历史回答下方。新消息不会自动继承旧任务，在计划模式中明确回复“继续某个任务”时，Agent 通过 `plan_resume` 恢复对应计划。
