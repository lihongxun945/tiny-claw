...
describe("MessageHistory", () => {
  ...
  it("counts user turns without removing tool messages", () => {
    const history = new MessageHistory([
      message("user", "u1"),
      {
        role: "assistant",
    ...
    expect(history.getRecentMessages(2)).toEqual([
      message("user", "u1"),
      { role: "assistant", content: [{ type: "tool_use", id: "call-1", name: "bash", input: { command: "pwd" } }] },
  ...
  it("does not let dense tool traffic evict the previous user prompt", () => {
    ...
    expect(history.getRecentMessages(1)).toEqual([
      message("user", "original-task"),
      { role: "assistant", content: [{ type: "text", text: "progress-1" }, { type: "tool_use", id: "call-1", name: "bash", input: { command: "one" } }] },
      { role: "user", content: [{ type: "tool_result", tool_use_id: "call-1", content: "large-result-1" }] },
      { role: "assistant", content: [{ type: "text", text: "progress-2" }, { type: "tool_use", id: "call-2", name: "bash", input: { command: "two" } }] },
      { role: "user", content: [{ type: "tool_result", tool_use_id: "call-2", content: "large-result-2" }] },
      message("assistant", "iteration-limit"),
      message("user", "continue"),
    ]);
  });

  it("preserves completed tool messages from previous turns", () => {
    const history = new MessageHistory([
      message("user", "previous-user"),
      {
        role: "assistant",
    ...
    expect(history.getRecentMessages(10)).toEqual([
      ...
      {
        role: "assistant",
        content: [{ type: "text", text: "I will search" }, { type: "tool_use", id: "call-1", name: "web_search", input: { query: "large" } }],
      },
      { role: "user", content: [{ type: "tool_result", tool_use_id: "call-1", content: "very large result" }] },
      message("assistant", "previous-final-answer"),
      message("user", "current-user"),
    ]);
  });
