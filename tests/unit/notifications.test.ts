...
describe("resolveNotification", () => {
  it("notifies default reasons when settings are absent", () => {
    expect(resolveNotification("completed")).toEqual({ title: "本轮完成", body: "Breeze Coder 已完成本轮任务，可回来查看结果" });
    expect(resolveNotification("approval_required")).toEqual({ title: "需要你的审批", body: "Breeze Coder 已暂停，等待你批准后继续执行" });
    expect(resolveNotification("waiting_user")).toEqual({ title: "等待你的输入", body: "Breeze Coder 已暂停，等待你的回答" });
    expect(resolveNotification("iteration_limit")).toEqual({ title: "已达迭代上限", body: "Breeze Coder 本轮已自动停止，可继续追问" });
  });
